...
set "scriptDir=%~dp0"
set "configFile=%scriptDir%open-app.json"
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$config = Get-Content -Raw -LiteralPath '%configFile%' | ConvertFrom-Json; $type = '%selectedType%'; $value = '%selectedValue%'; $ids = if ($type -eq 'P') { @($value) } else { $value -split ',' }; foreach ($id in $ids) { $project = @($config.projects | Where-Object { $_.id -eq [int]$id })[0]; if ($null -ne $project) { switch ($project.openin) { 'vscode' { Start-Process code -ArgumentList ('"' + $project.path + '"') }; 'vstudio' { Start-Process devenv -ArgumentList ('"' + $project.path + '"') }; 'notepad' { Start-Process notepad -ArgumentList ('"' + $project.path + '"') }; 'explorer' { Start-Process explorer -ArgumentList ('"' + $project.path + '"') }; default { Write-Error ('Executor nao suportado: ' + $project.openin) } } } }"
